...
class GCN:
    ...
    def cnnlayer(self,inputdata):
        with tf.variable_scope("cnn"):
            w = tf.get_variable('w', shape=[1,64, 2, 16], dtype=tf.float32, initializer=
            tf.truncated_normal_initializer(stddev=0.01))
            b = tf.get_variable('b', shape=[16], dtype=tf.float32, initializer=
            tf.zeros_initializer)
            w1 = tf.get_variable('w1', shape=[1, 64, 16, 8], dtype=tf.float32, initializer=
            tf.truncated_normal_initializer(stddev=0.01))
            b1 = tf.get_variable('b1', shape=[8], dtype=tf.float32, initializer=
            tf.zeros_initializer)
            h = tf.nn.relu(tf.nn.conv2d(inputdata, w, strides=[1, 1, 1, 1], padding="SAME") + b)
    ...
    def classification(self, inputdata, keep_prob):
        with tf.variable_scope("classi"):
            w1 = tf.get_variable('w1', shape=[9728, 300], initializer=
            ...
            inputdata = tf.reshape(inputdata, (-1, 9728))
