...
class GCN:
    ...
    def Convlayer(self, inputdata, lapal, keep_prob, dim, shape, act=True):
        with tf.variable_scope("layer" + str(dim)):
            ...
            if act:
                ...
                return tf.reshape(tf.add_n(supports), (-1, self.adjnum, shape[1]))
    ...
    def train(self):
        ...
        gcnlayer2 = self.Convlayer(gcnlayer1, laplamatr, dropout, 2, [self.outdim, self.outdim], False)
        classificationlayer = self.classification(gcnlayer2, dropout)
        ...
        train_op = tf.train.AdamOptimizer(self.lr, beta1=0.5).minimize(allloss, var_list=[var_1 + var_3])
