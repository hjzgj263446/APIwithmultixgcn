...
def Convlayer(inputdata, lapal, keep_prob):
    with tf.variable_scope("layer"):
        w1 = tf.get_variable('w1', shape=[adjnum, 128], initializer=
        ...
        lapal1 = tf.reshape(tf.matmul(lapal, inputdata), [tf.shape(lapal)[0] * adjnum, adjnum])
        h1 = tf.nn.relu(tf.matmul(lapal1, w1))
        h1 = tf.layers.dropout(h1, keep_prob)
        h1 = tf.reshape(h1, [tf.shape(lapal)[0], adjnum, 128])
        temp = tf.matmul(lapal, h1)
        temp = tf.reshape(temp, [tf.shape(temp)[0] * adjnum, 128])
        h2 = tf.nn.relu(tf.matmul(temp, w2))  # shape:[batch_size*adjnum,128]
...
def attention(inputdata):
    with tf.variable_scope("attention"):
        w1 = tf.get_variable('w1', shape=[128, 1], initializer=
        tf.truncated_normal_initializer(stddev=0.1))
        b1 = tf.get_variable('b1', shape=[1], initializer=tf.zeros_initializer)
        h1 = tf.matmul(inputdata, w1) + b1
        h1 = tf.reshape(h1, (-1, adjnum))
        h1 = tf.nn.softmax(h1)
        h1 = tf.reshape(h1, (-1, 1, adjnum))
        tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(0.001)(w1))
        return h1


def classification(inputdata, attention):
    with tf.variable_scope("classi"):
        w1 = tf.get_variable('w1', shape=[128, 8], initializer=
        tf.truncated_normal_initializer(stddev=0.1))
        b1 = tf.get_variable('b1', shape=[8], initializer=tf.zeros_initializer)
        tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(0.001)(w1))
        inputdata = tf.reshape(inputdata, (-1, adjnum, 128))
        h1 = tf.matmul(attention, inputdata)
        h = tf.squeeze(h1)
        h = tf.reshape(h, (-1, 128))
        h2 = tf.matmul(h, w1) + b1
        return h2


def getXY(file, label, dataindex, i, j, size, istrain):
    temp = np.zeros((size, adjnum, adjnum))
...
def getTestdata(i):
    with open("laplamatrix_noneI_test" + str(i) + ".pkl", "rb") as f1:
        ...
        temp = np.zeros((length, adjnum, adjnum))
        count = 0
...
lr = 1e-3
batch_size = 20
laplamatr = tf.placeholder(tf.float32, shape=[None, adjnum, adjnum])
x = tf.placeholder(tf.float32, shape=[None, adjnum, adjnum])
y = tf.placeholder(tf.int32, shape=[None, 1])
dropout = tf.placeholder(tf.float32)
y_onehot = tf.one_hot(y, 8)
y_onehot = tf.reshape(y_onehot, (-1, 8))
gcnlayer = Convlayer(x, laplamatr, dropout)
attentionlayer = attention(gcnlayer)
classificationlayer = classification(gcnlayer, attentionlayer)
